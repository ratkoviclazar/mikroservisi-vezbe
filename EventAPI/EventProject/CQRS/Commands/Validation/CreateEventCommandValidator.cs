...
{
    public sealed class CreateEventCommandValidator : ICommandValidator<CreateEventCommand>
    {
        private readonly IEventsWriteStore _writeStore;

        public CreateEventCommandValidator(IEventsWriteStore writeStore)
        {
            _writeStore = writeStore;
        }

        public async Task<IReadOnlyList<string>> ValidateAsync(CreateEventCommand command, CancellationToken cancellationToken = default)
        {
